...
Это целевая структура; сейчас созданы лишь независимые доменные/application-модули Z и их тесты, полный устанавливаемый пакет ещё предстоит собрать. [Фактическое состояние](../../context/current-state.md). Один устанавливаемый Python-пакет будет содержать все слои; отдельные процессы импортируют общие contracts. Contracts и миграции имеют одного ответственного при объединении треков; остальные модули работают через согласованные интерфейсы.
