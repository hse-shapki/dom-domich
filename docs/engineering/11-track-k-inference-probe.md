...
- В локальный cache вне репозитория загружен `Qwen3-8B-Q4_K_M.gguf`, revision
...
- Загружен `Qwen3-Embedding-0.6B-Q8_0.gguf`, revision
...
- Текстовая модель с полным Metal offload завершилась ошибкой нехватки GPU
  ...
  выполняется. G0 inference остаётся открытым до проверки на другом хосте.
