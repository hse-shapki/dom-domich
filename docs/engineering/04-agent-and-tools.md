...
> Live модель и общий process entrypoint с production Z ports ещё отсутствуют.
...
K07 `PostgresCaseReader` проверен на локальном PostgreSQL 18: жёсткие фильтры дома,
подъезда, этажа и объекта отсекают очевидно чужие дела; закрытые более 30 дней
не возвращаются. Русская FTS и структурное совпадение дают top-10 кандидатов.
Опциональная vector-ветка включается только при установленном pgvector и
совпадающей embedding revision; она пока не проверена на целевом PG17/pgvector.
...
K08: `PostgresCaseWriter` сериализует один idempotency key и fingerprint
локации, повторно проверяет активное дело под коротким advisory lock, сохраняет
case/message/event/operation вместе. Повтор с тем же ключом и другими
аргументами отклоняется, а новая проблема после закрытия получает
`recurrence_of`. Source message для resident write сверяется с доверенным
`event_id`; одно событие может ссылаться на несколько разных дел. Это
проверено на локальном PostgreSQL 18, но Z workflow ещё не подключён.
...
ответственным, применимостью и точкой отсчёта. На локальном PG18 проверена FTS;
