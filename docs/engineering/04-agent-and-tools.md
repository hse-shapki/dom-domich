...
Knowledge ingestion: разрешённый источник → текст → chunks с source/revision → embedding → FTS/vector index. Для нормативного действия нужен не просто похожий фрагмент, а проверенная rule entry с условиями применимости, временной базой и источником.
