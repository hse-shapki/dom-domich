...
[Callback handler Z05](../../src/dom_domych/application/polls/callback.py) читает серверную запись непрозрачного action token, проверяет дом, реального actor, привязку к жителю, subject revision и срок, затем вызывает тот же PollService. Fake-событие и хранилище проверены; A10 должен передать нормализованный MAX callback/actor и хранить/отзывать action tokens. Старые кнопки после изменения инициативы должны отзываться Z06, а не превращаться в голос за новую редакцию.

[InitiativeService Z06](../../src/dom_domych/application/initiatives/service.py) использует предлагаемый K `CasePort.get_case`, immutable `InitiativeRevision` и репозиторный метод `revise_and_replace_poll_atomic`. В production один commit должен сохранить новую редакцию, отменить старый poll, отозвать его action tokens, открыть новый poll и записать уведомления/job. В fake это уже проверено; общий контракт K/A и PostgreSQL реализация остаются открытыми.
