...
Персональные задачи, владение файлами и доказательства gates: [план реализации](../../context/implementation-plan.md). Здесь архитектурные треки **A/B/C**; в персональном плане они обозначены **A/K/Z**: A — Алина, B/K — Катерина, C/Z — Замира. Этот документ объясняет общий порядок; конкретный task для работы берётся из персонального пула.
...
**Владелец:** Алина. Основные области: `src/dom_domych/entrypoints`, infrastructure/max, postgres runtime, deploy, residents. Она выступает интегратором Alembic-миграций; модули Катерины и Замиры владеют своими ORM-моделями и revisions.
...
**Владелец:** Катерина. Области: `src/dom_domych/agent`, cases, requests, knowledge, rule engine; contracts tools согласовывает с соседями.
...
**Владелец:** Замира. Области: audiences, polls, initiatives, documents, resolution, demo executor. Использует MAX port трека A.
...
- Каждый task и его конкретная проверка находятся в персональном [плане](../../context/implementation-plan.md); пока соседний модуль не готов, работа идёт через утверждённый port/fake. G0 фиксирует границы, G1–G3 и Release подтверждают связанные сценарии по [08](08-verification-and-release.md).
