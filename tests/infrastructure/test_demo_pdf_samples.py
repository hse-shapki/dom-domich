...
def test_demo_pdf_samples_cover_four_kinds_with_synthetic_facts() -> None:
    ...
    assert all(item.tally is not None and item.tally.eligible == 12 for item in snapshots)
    register = next(item for item in snapshots if item.kind is DocumentKind.NOTIFICATION_REGISTER)
