...
| Python-проект, зависимости и CI A00 | Проверено локально; CI ещё не запускался на GitHub | `pyproject.toml`, `uv.lock`, `.python-version`, `.github/workflows/python.yml`; 225 тестов, Ruff и mypy проходят |
...
| Deploy/runtime A12 | Частично: Compose запускает API/inbox/scheduler/outbox/retention; K runtime использует настраиваемый llama-server endpoint | `Dockerfile`, `deploy/compose.yml`, `entrypoints/processes.py`; образ модели, daemon/HTTPS/restart volumes и live inference не проверены |
