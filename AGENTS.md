...
**Контекст проекта должен сохраняться в файлах.** Окно текущей беседы помогает вести работу, но не является единственным хранилищем решений, статуса и важных ограничений. Перед каждой задачей перечитывай `context/README.md`, `decisions.md`, `current-state.md`, `product.md` и `engineering.md`; после значимого решения или изменения реализации обновляй относящиеся к нему файлы в том же коммите. После смены задачи или сжатия окна восстанови контекст из этих файлов и проверь актуальное состояние репозитория. Не копируй в них секреты и частные данные.
...
Команда из трёх человек ведёт реализацию через AI. Порядок и параллельные треки описаны в [плане работ](docs/engineering/07-workstreams.md).
...
| Три трека разработки и критерии приёмки | План подготовлен | `docs/engineering/07-workstreams.md`, `08-verification-and-release.md` |
...
2. Перечитай всю папку `context/`, начиная с её README, затем относящуюся к задаче спецификацию. Полные правила находятся в `docs/engineering`, здесь — общий стандарт. Не полагайся только на память из окна беседы.
3. Выполни текущую задачу в её объёме. Не создавай пустые модули всего roadmap для имитации прогресса.
4. При параллельной работе соблюдай границы треков A/B/C; согласовывай shared contracts и миграции перед объединением, не плодя несовместимые версии.
