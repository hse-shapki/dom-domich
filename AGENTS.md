...
| Composition/polling A14 | Частично: polling/dispatcher/revision router и K message/continuation composition реализованы | `application/agent/composition.py`, `infrastructure/max/polling.py`, dispatcher/scheduler; process root, Z repositories и общий G3 runtime отсутствуют |
...
| K00: стыки агента, дел и знаний | Проверено на contract/fake уровне; единый реестр восьми K tools связан с runtime, production services ещё не собраны | `src/dom_domych/agent/contracts.py`, `fakes.py`, `tool_handlers.py`, `tools.py`, K00/K03 tests |
...
| K03: agent runtime | Частично: allowlist/schema/capability/budget и восемь K tools собраны с production K services; live inference не проверен | `src/dom_domych/agent/runtime.py`, `agent/tools.py`, `application/agent/composition.py`, K03/K14 tests |
