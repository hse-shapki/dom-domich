...
| K14: сквозная проверка | Частично: A scheduler → production K composition → deadline → agent run проверены на PostgreSQL 18 с fake LLM/Z ports, включая stale и чужой дом; полный Z poll/PDF/MAX цикл отсутствует | `tests/infrastructure/test_k14_deadline_cycle.py`, K08–K13 tests |
