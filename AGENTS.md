...
| K03: agent runtime | Частично: allowlist/schema/capability/budget, общий ToolResult и реестр K handlers проверены fake tests; production services ещё не собраны | `src/dom_domych/agent/runtime.py`, `agent/tools.py`, K03 tests |
...
| K13: события продолжения | Частично: восемь событий связаны с fresh run; `request.status_changed` и `document.ready` сначала проверяются через Z ports и атомарно обновляют K state, `done` не закрывает дело; Z producers, общий процесс и live inference ещё не подключены | `application/agent/events.py`, `application/requests/events.py`, `infrastructure/postgres/requests.py`, K09/K10/K13 tests |
