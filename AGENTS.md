...
| InitiativeService Z06 | Частично, K case/author reader проверен на PostgreSQL 18, Z repository остаётся fake | `infrastructure/postgres/initiative_cases.py`, `application/initiatives/service.py`, tests; нет PostgreSQL UoW инициатив/опросов и реального отзыва MAX action tokens |
...
| Python-проект, зависимости и CI A00 | Проверено локально; CI ещё не запускался на GitHub | `pyproject.toml`, `uv.lock`, `.python-version`, `.github/workflows/python.yml`; 227 тестов, Ruff и mypy проходят |
| PostgreSQL core A02 | Частично: проверено на локальном PostgreSQL 18, PG17/pgvector не запущен | `migrations/`, `infrastructure/postgres/`, `scripts/seed_demo_house.py`; миграция и повторный seed без дублей; 26.09.2026 Docker daemon доступен, но pull целевого образа дважды завершился TLS timeout Docker Hub |
...
| Inbox worker A07 | Частично: lease/recovery/retry и K message handler проверены на PostgreSQL 18 | `application/jobs/inbox_worker.py`, `application/agent/messages.py`, K14/inbox tests; отдельный production-процесс и Z handlers ещё не подключены |
...
| Deploy/runtime A12 | Частично: Compose запускает API/inbox/scheduler/outbox/retention; K runtime использует настраиваемый llama-server endpoint; readiness различает БД и деградацию LLM | `Dockerfile`, `deploy/compose.yml`, `entrypoints/api.py`, `entrypoints/processes.py`; образ модели, HTTPS/restart volumes и live inference не проверены |
...
| Composition/polling A14 | Частично: process root запускает K message/continuation inbox и K scheduler; Z executor/document отсутствуют и явно fail, не имитируются | `entrypoints/processes.py`, `application/agent/composition.py`, process/config tests; Z repositories/callback/PDF jobs и общий G3 runtime отсутствуют |
...
| K11: emergency | Частично: typed agent tool без poll, критичное уточнение и личный evidence outbox собраны с production K services и проверены на PostgreSQL 18; live MAX и production executor отсутствуют | `application/requests/emergency.py`, `agent/tools.py`, `infrastructure/postgres/emergency_evidence.py`, K11 tests |
...
| K13: события продолжения | Частично: восемь событий связаны с fresh run; mutation handlers регистрируются раньше agent, `done` не закрывает дело; Z producers, общий process и live inference ещё не подключены | `application/agent/events.py`, `application/requests/events.py`, `application/agent/composition.py`, K09/K10/K13/K14 tests |
| K14: сквозная проверка | Частично: MAX event/inbox → trusted triage → production case tool → run/reply outbox и A scheduler → K deadline проверены на PostgreSQL 18 с fake LLM/Z ports; полный Z poll/PDF/live MAX цикл отсутствует | `tests/infrastructure/test_k14_message_cycle.py`, `test_k14_deadline_cycle.py`, K08–K13 tests |
