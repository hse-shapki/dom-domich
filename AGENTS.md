...
| K13: события продолжения | Частично: восемь доверенных событий связаны с новым run; `request.registered` и `evidence.added` пишутся в durable inbox, регистрация проходит через worker к свежему делу на PostgreSQL 16; остальные producers, общий процесс и live inference ещё не подключены | `application/agent/events.py`, `infrastructure/postgres/requests.py`, `case_evidence.py`, K09/K10/K13 tests |
