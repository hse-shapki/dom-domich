...
| K13: события продолжения | Частично: восемь доверенных событий связаны с новым run; `request.registered` идёт в durable inbox и через worker к свежему делу на PostgreSQL 16; остальные producers, общий процесс и live inference ещё не подключены | `application/agent/events.py`, `infrastructure/postgres/requests.py`, K10/K13 tests |
